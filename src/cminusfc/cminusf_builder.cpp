...
Value* CminusfBuilder::visit(ASTVarDeclaration &node) 
{
    ...
    if (is_array) {
        var_type = ArrayType::get(var_type, node.num->i_val);
    }

    Value* allocation = nullptr;
    if (scope.in_global()) {
        auto initializer = ConstantZero::get(var_type, module.get());
        allocation = GlobalVariable::create(node.id, module.get(), var_type, false, initializer);
    } else {
        allocation = builder->create_alloca(var_type);
    }

    scope.push(node.id, allocation);
    return nullptr;
}
...
{
    bool created_new_scope = !pre_enter_scope;
    if (pre_enter_scope) {
    ...
    if (created_new_scope) scope.exit();
    return nullptr;
}
...
{
    BasicBlock* cond_bb = BasicBlock::create(module.get(), "loop_cond", cur_fun);
    BasicBlock* body_bb = BasicBlock::create(module.get(), "loop_body", cur_fun);
    BasicBlock* exit_bb = BasicBlock::create(module.get(), "loop_exit", cur_fun);
